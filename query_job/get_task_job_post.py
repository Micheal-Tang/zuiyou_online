...
class Gettask(object):
    ...
    def __init__(self):
        ...
        self.script_path = "/home/devs/tangyongjun/post/script/query_post.py"
        self.output_path = "/home/devs/tangyongjun/post/result"

    def get_task_id(self):
        docs_zy = self.table_zy.find({"toolkit_type":"post","type": 1,"status": {"$in": [1,2]}})
        docs_pp = self.table_pp.find({"toolkit_type": "post", "type": 1, "status": {"$in": [1, 2]}})
        docs_omg = self.table_omg.find({"toolkit_type": "post", "type": 1, "status": {"$in": [1, 2]}})
        docs_maga = self.table_maga.find({"toolkit_type": "post", "type": 1, "status": {"$in": [1, 2]}})
